...
from django.http import HttpResponse, HttpResponseNotFound # Для повернення простого респонзу та помилки 404
...
def profile_view(request):
    current_teacher = Teacher.objects.all().filter(user_id=request.user) # Пробуємо знайти об'єкт вчителя з user_id == request.user
    if current_teacher: # Якщо знайшли
        subjects = current_teacher[0].subjects.all() # Отримуємо всі предмети, які викладає вчитель
        courses = Course.objects.filter(users__in=[request.user]) # Всі курси, які він веде
        ...
        current_student = Student.objects.get(user_id=request.user) # Отримуємо відповідний об'єкт
        courses = Course.objects.filter(users__in=[request.user]) # Та всі курси, на яких він вчиться
...
class ProfileEditView(View):
    def get(self, request):
        if not request.user.is_authenticated:
            return redirect('login') # Якщо юзер не ввійшов в акаунт - відправляємо логінитися
...
class NewCourseView(View):
    def get(self, request):
        ...
        if request.user.status != 'T': # Якщо юзер не вчитель - не пускаємо на сторінку створення курсу
    ...
    def post(self, request):
        ...
        new_course.users.add(request.user) # Одразу додаємо вчителя, який його створив як нового користувача
...
class SubjectAddingView(View):
    def get(self, request):
        ...
        if request.user.status != 'T':
            return HttpResponseNotFound()
        return render(request, 'testapp/add_subject.html') # До створення нового предмету допускаємо лише вчителів

    def post(self, request):
        if request.user.status == 'T':
            new_sub = request.POST.get('subject') # Беремо назву предмету з поля
            try:
                created_sub = Subject.objects.get(subject_name=new_sub) # Пробуємо взяти такий об'єкт з бази даних
            except:
                created_sub = Subject(subject_name=new_sub)
                created_sub.save() # Якщо не виходить - створюємо новий та зберігаємо
            if request.POST.get('add'):
                created_sub.teachers.add(Teacher.objects.get(user_id=request.user)) # Якщо вчитель не прибрав галочку - одразу додаємо до його предметів
        return redirect('profile')
...
class JoinCourseView(View):
    def get(self, request):
        if request.user.is_authenticated:
            return render(request, 'testapp/enter_course.html') # До курсу допускаємо лише аутентифікованих користувачів
        else:
            return redirect('login')
    ...
    def post(self, request):
        ...
        try:
            ...
            return redirect('course', code=code) # Шукаємо його, додаємо туди користувача та перекидаємо на сторінку курсу
        except:
            return HttpResponse('Курс не знайдено') # Якщо не знаходимо - повідомляємо користувачу
...
class CourseView(View):
    def get(self, request, code):
        ...
        try:
            ...
            return HttpResponseNotFound()
        tusers = course.users.all().filter(status='T') # Отримуємо перелік вчителів (users)
        teachers = []
        for user in tusers:
        ...
        # Рендеримо сторінку      
        ctx = {'course' : course, 'teachers' : teachers, 'present' : present_tests,
    ...
    def post(self, request, code):
        ...
        for quetion in json.loads(request.POST.get('q')): # Та об'єкти питань та відповідей
            ...
            for answer in quetion.get('ans'):
                ...
                a.save()
        return HttpResponse() # Даємо звичайний респонз, JS все одно нас на курс перекине
...
def courseoverview(request, code):
    ...
    if request.GET.get('page'):
        ...
        page = 1
    tests = cut_by_page(tests, page) # Вирізаємо шматочок кверісету тестів за нашою сторінкою
